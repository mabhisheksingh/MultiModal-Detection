...
def handle_list_buckets(client: CephClient, args):
    ...
    try:
        ...
        table_data = [[bucket["name"], bucket["creation_date"].strftime("%Y-%m-%d %H:%M:%S")] for bucket in buckets]
...
def handle_upload(client: CephClient, args):
    ...
    try:
        ...
        client.upload_file(file_path, bucket, object_name=object_name, metadata=metadata if metadata else None)
...
def handle_upload_uuid(client: CephClient, args):
    ...
    try:
        ...
        object_store_id = client.save_file_into_ceph(file_path, bucket, metadata=metadata if metadata else None)
...
Examples:
  ...
  %(prog)s info --object file.txt --bucket my-bucket
    ...
    delete_bucket_parser.add_argument("--force", action="store_true", help="Force delete (remove all objects first)")
    ...
    upload_parser.add_argument(
        "--metadata", nargs="*", help="Metadata in key=value format (e.g., author=john version=1.0)"
    )
    ...
    upload_uuid_parser.add_argument(
        "--metadata", nargs="*", help="Metadata in key=value format (e.g., author=john version=1.0)"
    )
    ...
    delete_file_parser.add_argument("--force", action="store_true", help="Skip confirmation prompt")
