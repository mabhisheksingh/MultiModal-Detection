...
class CephClient:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            boto_config = Config(
                ...
                retries={"max_attempts": self.config.get("retry", {}).get("max_attempts", 3), "mode": "standard"},
            )
    ...
    def delete_bucket(self, bucket_name: str, force: bool = False) -> bool:
        ...
        try:
            ...
            if force:
                self.logger.info("Force delete enabled, removing all objects first")
            ...
            if error_code == "NoSuchBucket":
                ...
                raise Exception(f"Bucket '{bucket_name}' is not empty. Use --force to delete all objects first")
            else:
    ...
    def upload_file(
        self, file_path: str, bucket_name: str, object_name: str | None = None, metadata: dict[str, str] | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            self.s3_client.upload_file(
                file_path, bucket_name, object_name, ExtraArgs=extra_args if extra_args else None
            )
    ...
    def save_file_into_ceph(self, file_path: str, bucket_name: str, metadata: dict[str, str] | None = None) -> str:
        """
        ...
        try:
            ...
            self.s3_client.put_object(Bucket=bucket_name, Key=object_store_id, Body=open(file_path, "rb"), **extra_args)
    ...
    def download_file(self, bucket_name: str, object_name: str, file_path: str | None = None) -> str:
        """
        ...
        try:
            self.logger.info(f"Downloading {bucket_name}/{object_name} to {file_path}")
            ...
            file_size = os.path.getsize(file_path)
            self.logger.info(f"Successfully downloaded: {file_path} ({format_size(file_size)})")
            return file_path
