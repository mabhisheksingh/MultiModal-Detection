...
class APIResponse:
    ...
    def success(data, status_code=200):
        ...
        return (
            jsonify({"success": True, "data": data, "timestamp": __import__("datetime").datetime.now().isoformat()}),
            status_code,
        )
...
class JobRequest:
    ...
    def validate(self):
        ...
        for i, inp in enumerate(self.inputs):
            ...
            elif input_type == "video_url" and "url" not in data:
...
class JobResponse:
    ...
    def to_dict(self):
        ...
        if self.job["status"] == JobStatus.PROCESSING:
            response.update({"current_stage": self.job.get("current_stage"), "started_at": self.job.get("started_at")})
