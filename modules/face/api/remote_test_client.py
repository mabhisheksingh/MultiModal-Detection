...
class RemoteFaceDetectionAPIClient:
    ...
    def submit_config(self, config_path):
        ...
        try:
            ...
            response = requests.post(
                f"{self.api_base}/process-config", json=config_data, timeout=60  # Longer timeout for remote requests
            )

            if response.status_code == 202:
                result = response.json()
                job_id = result["job_id"]
                print("✅ Job submitted to GPU server successfully!")
    ...
    def monitor_job(self, job_id, check_interval=20):
        ...
        while True:
            ...
            if percentage != last_percentage:
                ...
                print(
                    f"🔄 [{elapsed:03d}s] GPU {current_status.upper()}: {percentage:3d}% - {status.get('current_stage', 'processing')}"
                )
                last_percentage = percentage

            if current_status == "completed":
                print("✅ Remote GPU job completed successfully!")
                print(f"   - Processing time: {status.get('processing_time', 'unknown')}")
                print(f"   - GPU Server: {self.server_ip}")
                return status

            elif current_status == "failed":
                print("❌ Remote GPU job failed!")
    ...
    def download_all_results(self, job_id, download_folder="remote_gpu_downloads"):
        ...
        try:
            print("📋 Getting results from GPU server...")
...
def main():
    ...
    if client.download_all_results(job_id):
        ...
        print("📁 Check the 'remote_gpu_downloads' folder for your files:")
        print("   - face_data_export.json (ArcFace embeddings from GPU)")
        print("   - processed_video.mp4 (GPU-accelerated processing)")
    else:
