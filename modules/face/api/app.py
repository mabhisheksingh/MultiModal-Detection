...
from models import APIResponse, JobStatus
...
def health_check():
    ...
    try:
        ...
        try:
            ...
            triton_status = "connected" if triton_client.is_server_ready() else "disconnected"
        except:
            triton_status = "error"
...
def process_config():
    ...
    try:
        ...
        processing_thread = Thread(target=_process_config_async, args=(job_id, config_data), daemon=True)
...
def _process_config_async(job_id, config_data):
    ...
    try:
        ...
        try:
            ...
            video_client.main(config_file=relative_temp_config)
        ...
        if os.path.exists(output_dir):
            print("📁 Output directory exists, scanning...")
        ...
        job_manager.complete_job(
            job_id,
            {
                ...
                "summary": {"outputs_generated": len(output_paths), "processing_duration_seconds": processing_time},
            },
        )
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=config.SERVER_PORT, debug=config.DEBUG_MODE, threaded=True)
