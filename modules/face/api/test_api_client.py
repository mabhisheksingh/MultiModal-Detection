...
class FaceDetectionAPIClient:
    ...
    def submit_config(self, config_path):
        ...
        try:
            ...
            response = requests.post(f"{self.api_base}/process-config", json=config_data, timeout=30)
    ...
    def monitor_job(self, job_id, check_interval=15):
        ...
        while True:
            ...
            if percentage != last_percentage:
                ...
                print(
                    f"🔄 [{elapsed:03d}s] {current_status.upper()}: {percentage:3d}% - {status.get('current_stage', 'processing')}"
                )
...
def main():
    ...
    if client.download_all_results(job_id):
        ...
        print("📁 Check the 'api_downloads' folder for your files:")
        print("   - face_data_export.json (same format as video_client.py)")
        print("   - processed_video.mp4 (with face tracking)")
    else:
