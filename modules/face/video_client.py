...
def initialize_embedding_generator(processing_config):
    ...
    try:
        ...
        print("✅ ArcFace embedding generator initialized")
        print(f"   Model: {embedding_generator.model_name}")
        print(
            f"   Embedding size: {embedding_generator.model_specs.get(embedding_generator.model_name, {}).get('embedding_size', 'Unknown')}"
        )
        return True
...
def generate_face_embedding(face_image, processing_config):
    ...
    if embedding_generator is None and processing_config.get("generate_embeddings", False):
        print("🔧 Debug: Attempting to initialize embedding generator (was None)")
        if not initialize_embedding_generator(processing_config):
            print("❌ Debug: Failed to initialize embedding generator")
            return None
        print("✅ Debug: Embedding generator initialized successfully")

    # Skip if embedding generation is disabled or generator unavailable
    if embedding_generator is None:
        print("⚠️ Debug: Skipping embedding - generator is None")
        return None

    if not processing_config.get("generate_embeddings", False):
        print("⚠️ Debug: Skipping embedding - generate_embeddings is False")
        return None
...
def load_input_configuration(config_file=CONFIG_FILE):
    ...
    try:
        ...
        print("⚙️ Processing config loaded:")
        ...
        return [
            {
                ...
                "data": {"path": r"input\Dashcam_Helps_Police_Track_Down_Offenders_Heavy_Penalty.mp4"},
            }
        ], fallback_processing_config
...
def validate_json_frame_data(data):
    ...
    for frame in data:
        ...
        if "frame_id" not in frame or "frame_path" not in frame:
            return False
        if not isinstance(frame["frame_path"], str) or not frame["frame_path"].startswith(
            ("http://", "https://", "s3a://")
        ):
            return False
...
def fetch_frame_from_url(frame_url):
    ...
    try:
        if frame_url.startswith("s3a://") and ceph_client is not None:
            ...
            print("📡 Using CephClient for S3A URL")
...
def process_json_frames(json_path, output_name, json_base_name, processing_config):
    ...
    for frame_data in frames_data:
        ...
        inputs = grpcclient.InferInput("RetinaFace::input_0", img.shape, "FP32")
        ...
        for i, (box, _score) in enumerate(zip(boxes, scores, strict=False)):
        ...
        tracked_faces = face_tracker.process_faces(orig, boxes, scores, frame_id, embeddings_data, frame_timestamp_ms)
...
def process_video(video_path, output_name, video_base_name, processing_config):
    ...
    print(
        f"Original FPS: {original_fps}, Output FPS: {fps} {'(custom setting)' if fps != original_fps else '(original)'}"
    )
    ...
    while True:
        ...
        for tracking_id, face_info in tracked_faces.items():
            ...
            # Draw text
            cv2.putText(orig, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
...
def process_video_bytes(video_bytes, output_name, video_base_name, metadata, processing_config):
    ...
    try:
        ...
        print("✅ Successfully processed video from bytes")
        return result
...
def process_frame_bytes_json(frame_bytes_data, output_name, json_base_name, metadata, processing_config):
    ...
    for frame_data in frames_data:
        ...
        tracked_faces = face_tracker.process_faces(orig, boxes, scores, frame_id, frame_timestamp_ms=frame_timestamp_ms)
        total_faces_detected += len(boxes)
...
def main(config_file=None):
    ...
    try:
        ...
        process_all_inputs(config_path)
    ...
    for input_config in inputs:
        ...
        try:
            if input_type == "video_file":
                ...
                print(f"✅ Saved processed video: {output_path}")
                print(
                    f"📁 JSON output directory: {os.path.abspath(os.path.dirname(output_path)) if output_path else 'N/A'}"
                )
                ...
                actual_output_path = process_frame_bytes_json(
                    frame_bytes_data, output_name, json_base_name, metadata, processing_config
                )
                output_path = actual_output_path
                ...
                if not video_bytes:
                    print("❌ No video bytes data provided")
                    continue
                ...
                print("Processing video from bytes array...")
                actual_output_path = process_video_bytes(
                    video_bytes, output_name, video_base_name, input_data, processing_config
                )
                output_path = actual_output_path
                ...
                if not frame_bytes_data:
                    print("❌ No frame bytes data provided")
                    continue
                ...
                print("Processing frames from direct bytes data...")
                actual_output_path = process_frame_bytes_json(
                    frame_bytes_data, output_name, json_base_name, input_data, processing_config
                )
                output_path = actual_output_path
