...
class AnalyticsUtils:
    ...
    @staticmethod
    def normalize_bbox_center(bbox_xyxy: list[float], frame_width: int, frame_height: int) -> tuple[float, float]:
    ...
    @staticmethod
    def compute_direction_vector(prev_center: tuple[float, float], curr_center: tuple[float, float]) -> list[float]:
    ...
    def compute_pixel_speed(
        ...
        if math.sqrt(dx * dx + dy * dy) < MIN_PIXEL_DISPLACEMENT_FOR_MOTION:
            return 0.0
        return AnalyticsUtils.compute_speed_estimate(prev_center, curr_center, time_delta_ms)

    @staticmethod
    def convert_pixel_speed_to_kmph(pixel_speed: float, pixels_per_meter: float | None) -> float:
    ...
    def format_velocity_display(
        ...
        speed_kmph = AnalyticsUtils.convert_pixel_speed_to_kmph(pixel_speed, pixels_per_meter)
        return [f"{speed_kmph:.1f} km/h"]
    ...
    @staticmethod
    def find_active_zone(center: tuple[float, float], zones: list[dict[str, Any]]) -> dict[str, Any] | None:
        for zone in zones:
