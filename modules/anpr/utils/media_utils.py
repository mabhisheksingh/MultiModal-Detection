...
class MediaSourceUtils:
    @staticmethod
    def validate_remote_media_url(url: str, *, source_name: str, allowed_extensions: list[str]) -> str:
        ...
        if suffix and suffix not in {ext.lower() for ext in allowed_extensions}:
            raise ValueError(f"Unsupported remote {source_name.split('_')[0]} extension: {url}")
        return normalized_url

    @staticmethod
    def resolve_camera_id(source: str, camera_id: str | None = None) -> str:
...
class VideoSourceUtils:
    ...
    def validate_video_source(source: str) -> None:
        ...
        if parsed.scheme.lower() in {"http", "https"}:
            suffix = Path(parsed.path).suffix.lower()
            if suffix and suffix not in {ext.lower() for ext in settings.video_extensions_list}:
                raise ValueError(f"Unsupported remote video extension for source: {source}")
            return
        FileSourceUtils.validate_file_path(source, {ext.lower() for ext in settings.video_extensions_list})

    @staticmethod
    def build_output_paths(source: str, output_dir: str, request_id: str | None) -> tuple[Path, str, str]:
        ...
        output_name = source_name if not is_remote_source else f"annotated_{Path(source_name).stem}.mp4"
...
class ImageSourceUtils:
    ...
    def validate_image_source_path(source_path: str) -> Path:
        return FileSourceUtils.validate_file_path(source_path, {ext.lower() for ext in settings.image_extensions_list})
    ...
    def validate_image_source_url(image_url: str) -> None:
        ...
        suffix = Path(parsed.path).suffix.lower()
        if suffix and suffix not in {ext.lower() for ext in settings.image_extensions_list}:
...
class ImageAnalysisUtils:
    ...
    def extract_dominant_color(image: np.ndarray, min_area: int = 2500) -> str:
        ...
        if neutral_pixels >= int(valid_pixels * 0.35):
            ...
            bright_ratio = float(np.count_nonzero(neutral_v >= 185)) / max(neutral_pixels, 1)
            dark_ratio = float(np.count_nonzero(neutral_v <= 70)) / max(neutral_pixels, 1)
