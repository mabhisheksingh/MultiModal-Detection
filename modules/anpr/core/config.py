...
class Settings(BaseSettings):
    ...
    # Model configuration
    vehicle_model_name: str = Field("vehicle_detection_rt_detr", validation_alias="VEHICLE_MODEL_NAME")
    vehicle_class_id_map: str = Field('{"0":"vehicle"}', validation_alias="VEHICLE_CLASS_ID_MAP")
    object_model_name: str = Field("object_detection", validation_alias="OBJECT_MODEL_NAME")
    object_class_id_map: str = Field('{"0":"person"}', validation_alias="OBJECT_CLASS_ID_MAP")
    plate_model_name: str = Field("plate_region_detection_rt_detr", validation_alias="PLATE_MODEL_NAME")
    plate_class_id_map: str = Field('{"0":"number_plate"}', validation_alias="PLATE_CLASS_ID_MAP")
    ...
    def plate_candidate_vehicle_classes_list(self) -> list[str]:
        return [name.strip().lower() for name in self.plate_candidate_vehicle_classes.split(",") if name.strip()]
    ...
    def _parse_class_id_map(self, raw_map: str, *, field_name: str) -> dict[int, str]:
        ...
        for key, value in parsed.items():
            try:
                ...
                raise ValueError(f"{field_name} keys must be numeric class IDs") from exc
    ...
    def vehicle_class_id_name_map(self) -> dict[int, str]:
        return self._parse_class_id_map(self.vehicle_class_id_map, field_name="VEHICLE_CLASS_ID_MAP")

    @computed_field
    @property
    def object_class_id_name_map(self) -> dict[int, str]:
        return self._parse_class_id_map(self.object_class_id_map, field_name="OBJECT_CLASS_ID_MAP")

    @computed_field
    @property
    def plate_class_id_name_map(self) -> dict[int, str]:
        return self._parse_class_id_map(self.plate_class_id_map, field_name="PLATE_CLASS_ID_MAP")
