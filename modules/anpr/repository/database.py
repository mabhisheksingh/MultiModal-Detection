...
def _ensure_sqlite_schema() -> None:
    ...
    column_names = {column["name"] for column in inspector.get_columns("global_identities")}
    if "license_plate_confidence" in column_names:
        return

    with engine.begin() as connection:
        connection.execute(
            text("ALTER TABLE global_identities " "ADD COLUMN license_plate_confidence FLOAT NOT NULL DEFAULT 0.0")
        )
    logger.info(
