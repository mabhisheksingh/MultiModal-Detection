...
class TritonClient:
    ...
    def _infer_request(
        ...
        infer_input = model.InferInput(model.input_names[0], [*input_batch.shape], np_to_triton_dtype(input_format))
        infer_input.set_data_from_numpy(input_batch)
        infer_inputs.append(infer_input)
        infer_outputs = [model.InferRequestedOutput(output_name) for output_name in model.output_names]
        ...
        return [outputs.as_numpy(output_name).astype(input_format) for output_name in model.output_names]

    async def infer(
        self, image: np.ndarray, frame_idx: int, request_id: str | None = None
    ) -> tuple[np.ndarray, dict[str, Any]]:
        try:
            model = self._get_model()
            preprocessed, preprocess_meta = self.preprocess_image(image, frame_idx)
            input_batch = np.expand_dims(preprocessed, axis=0)
            outputs = await asyncio.to_thread(self._infer_request, model, input_batch, request_id)
