...
from triton_server.app import GlobalTrackRepository, settings
...
class GlobalTrackingService:
    ...
    def __init__(self, repository: GlobalTrackRepository | None = None) -> None:
    ...
    def resolve(self, features: TrackFeatures) -> GlobalMatchResult:
        ...
        for identity in candidates:
            ...
            if features.license_plate_text and identity.license_plate_text and plate_similarity < plate_threshold:
                continue
            identity_plate_confidence = float(getattr(identity, "license_plate_confidence", 0.0) or 0.0)
            identity_last_seen_epoch = float(getattr(identity, "last_seen_epoch", 0.0) or 0.0)
            ...
            if not should_replace and score >= match_threshold and best_id is not None:
                ...
                if same_score and strong_plate_match:
                    if identity_plate_confidence > best_plate_confidence or (
    ...
    def _refresh_existing_association(self, association, features: TrackFeatures) -> GlobalMatchResult:
