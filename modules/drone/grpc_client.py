...
class TritonClient:
    ...
    def infer(self, model_name: str, input_tensor: np.ndarray) -> np.ndarray | None:
        """Run inference on a model"""
        if not self.connected:
            return None
        try:
            # Create input
            input_tensor_obj = grpc_client.InferInput(INPUT_NAME, input_tensor.shape, "FP32")
...
class ImageProcessor:
    ...
    @staticmethod
    def draw_detections(image: np.ndarray, detections: list[dict], confidence_threshold: float = 0.5) -> np.ndarray:
...
class DroneDetectionClient:
    ...
    def process_single_image(
        self, image_path: Path, model_name: str = MODEL_NAME, confidence_threshold: float = CONFIDENCE_THRESHOLD
    ) -> dict:
    ...
    def process_directory(
        self, input_dir: Path, model_name: str = MODEL_NAME, confidence_threshold: float = CONFIDENCE_THRESHOLD
    ) -> list[dict]:
        ...
        try:
            ...
            for i in range(detections_raw.shape[0]):
                ...
                if confidence > CONFIDENCE_THRESHOLD:
                    detections.append({"bbox": bbox.tolist(), "confidence": confidence, "label": "object"})
        except Exception as e:
