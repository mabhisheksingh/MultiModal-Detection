...
def test_score_with_adaptive_weights_and_class_mismatch():
    ...
    mismatch_score, mismatch_reasons = GlobalTrackingService._score(features, mismatch_identity)
...
def test_refresh_existing_association_path():
    repo = _FakeRepo()
    repo.association = _Association(id=99, global_id="gid_existing", match_score=0.8, match_reason="class+plate")
    service = GlobalTrackingService(repository=repo)
...
def test_refresh_existing_association_when_identity_missing_returns_existing_values():
    repo = _FakeRepo()
    repo.association = _Association(id=1, global_id="gid_missing", match_score=0.5, match_reason="existing")
...
def test_global_track_repository_helpers_and_crud():
    ...
    found = repo.find_association(camera_id="cam_1", local_track_id="11", request_id="req_1")
