...
def test_build_vehicle_episodes_splits_by_gap():
    ...
    episodes = AnalyticsUtils.build_vehicle_episodes(detections, reappearance_gap_ms=3000)
...
def test_media_source_validation_helpers(tmp_path: Path):
    ...
    with pytest.raises(ValueError):
        VideoSourceUtils.validate_video_source("https://example.com/video.unsupported_ext")
...
def test_request_trace_utils_build_triton_request_id():
    ...
    assert RequestTraceUtils.build_triton_request_id("req_1", "cam/a\\b test", 7) == "req_1:cam_a_b_test:7"
    assert RequestTraceUtils.build_triton_request_id("req_1", None, 0) == "req_1:unknown_source:0"
...
def test_ocr_utils_parse_and_normalize():
    ...
    assert OCRUtils.validate_plate_text(normalized, mode="strict")

    normalized = OCRUtils.normalize_plate_text(' "mp-09" ')
    assert normalized == "MP09"
    assert OCRUtils.validate_plate_text(normalized, mode="balanced")
    ...
    assert not OCRUtils.validate_plate_text(normalized, mode="strict")
    assert OCRUtils.validate_plate_text(normalized, mode="balanced")

    assert OCRUtils.normalize_plate_text("") == ""
    assert not OCRUtils.validate_plate_text("", mode="balanced")
...
def test_image_analysis_utils_remaining_color_branches():
    ...
    with patch("modules.anpr.utils.media_utils.cv2.calcHist", return_value=np.zeros((180, 1), dtype=np.float32)):
    ...
    with (
        patch.object(ImageAnalysisUtils, "_VALID_PLATE_COLORS", {"white", "yellow", "green", "black"}),
        patch.object(ImageAnalysisUtils, "extract_dominant_color", return_value="grey"),
    ):
    ...
    with patch("modules.anpr.utils.media_utils.cv2.cvtColor", return_value=np.zeros((0, 0, 3), dtype=np.uint8)):
...
def test_build_detection_response_item_and_skipped_csv_paths(tmp_path: Path):
    ...
    no_plate_rows = aggregate_track_votes([{"track_id": "10", "ocr_text": "", "conf": 0.1, "ocr_confidence": 0.1}])
...
def test_output_serializers_error_paths_return_none(tmp_path: Path):
    ...
    with patch("modules.anpr.utils.output_serializers.open", side_effect=OSError("nope")):
...
def test_image_source_utils_url_validation_and_load_paths():
    from triton_server.app import ImageSourceUtils
    ...
    with (
        patch("modules.anpr.utils.media_utils.urlopen", return_value=_Resp()),
        patch(
            "modules.anpr.utils.media_utils.cv2.imdecode",
    ...
    with (
        patch("modules.anpr.utils.media_utils.urlopen", return_value=_Resp()),
        patch("modules.anpr.utils.media_utils.cv2.imdecode", return_value=None),
